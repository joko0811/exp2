...
public class TermDiv extends CParseRule {
	...
	public void semanticCheck(CParseContext pcx) throws FatalErrorException {
		...
		if (left != null && right != null) {
			...
			if (nt == CType.T_err) {
				pcx.fatalError(op.toExplainString() + "左辺の型[" + left.getCType().toString() + "]と右辺の型[" + right.getCType().toString() + "]は割ることができません");
			}
	...
	public void codeGen(CParseContext pcx) throws FatalErrorException {
		...
		if (left != null && right != null) {
			...
			o.println("\tJSR\tDIV\t; TermDiv: DIVサブルーチンを呼び出し、引数除去・結果の取り出しを行う<" + op.toString() + ">");
			o.println("\tSUB\t#2, R6\t; TermDiv: 引数除去");
